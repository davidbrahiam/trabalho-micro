...
//********************************************************************
//variáveis globais
volatile unsigned char Dsp1=0;						//declaração  de variável local inicializada
volatile unsigned char Dsp2=0;						//declaração  de variável local inicializada
volatile unsigned char Dsp3=0;						//declaração  de variável local inicializada
volatile unsigned char Dsp4=0;						//declaração  de variável local inicializada
//********************************************************************/
#pragma code high_vector=0x08						//vetor de interrupção de alta prioridade
...
void high_isr (void)
{
	if(!INTCONbits.INT0IF);							//interrupção externa?
	else
	{
		INTCONbits.INT0IF = 0;						//sim, limpa bit de sinalização
 ...
 void Inic_Regs (void)
{
	...
	TRISB = 0x01;									//RB0 como entrada e demais pinnos do PORTB como saída
...
//habilita interrupção externa
	...
	INTCONbits.INT0IE = 1;							//liga chave individual de interrupção externa 0
	INTCON2bits.INTEDG0 = 0;						//interrupção externa 0 ocorrerá na borda de descida
//********************************************************************
}
